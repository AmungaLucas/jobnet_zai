...
import { sanitizeString, containsDangerousContent, escapeHtml } from '@/lib/sanitize';
import { addSecurityHeaders } from '@/lib/security';

// Maximum payload size (in characters)
const MAX_PAYLOAD_SIZE = 10000;

// GET - List organizations (public, but rate limited)
...
export async function POST(request) {
  try {
    ...
    const featured_image = sanitizeString(body.featured_image, { maxLength: 500 });
    const canonical_url = sanitizeString(body.canonical_url, { maxLength: 500 });
    const meta_title = sanitizeString(body.meta_title, { maxLength: 255 });
    const meta_description = sanitizeString(body.meta_description, { maxLength: 500 });
