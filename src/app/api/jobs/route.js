...
export async function GET(request) {
  try {
    ...
    // Get total count
    const [countResult] = await pool.query(`SELECT COUNT(*) as total FROM jobs j ${whereClause}`, queryParams);
    const total = countResult[0].total;

    // Get jobs with organization info
    const [jobs] = await pool.query(
      `SELECT j.*, o.organization_name, o.organization_logo_url
       FROM jobs j LEFT JOIN organizations o ON j.organization_id = o.id
       ${whereClause} ORDER BY j.${sortBy} ${sortOrder} LIMIT ? OFFSET ?`,
...
export async function POST(request) {
  try {
    ...
    const id = generateId();
    const slug = job_title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') + '-' + Date.now();
    ...
    const [newJob] = await pool.query(
      `SELECT j.*, o.organization_name, o.organization_logo_url
       FROM jobs j LEFT JOIN organizations o ON j.organization_id = o.id WHERE j.id = ?`,
      [id]
    );
